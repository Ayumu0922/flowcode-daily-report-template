  ...
  return (
    <PageTransition className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-foreground">今日の日報</h1>
        {templates.length > 0 && (
          <select onChange={(e) => loadTemplate(e.target.value)} defaultValue="" className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-foreground">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-3">
          <h2 className="text-sm font-semibold text-foreground mb-2">今日のタスク</h2>
          {tasks.map((task, i) => (
            <div key={i} className="flex items-center gap-3">
              <input value={task.title} onChange={(e) => updateTask(i, { title: e.target.value })} placeholder="タスク内容"
                className="flex-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-foreground placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-accent-500/50" />
              <select value={task.status} onChange={(e) => updateTask(i, { status: e.target.value as any })}
                className="bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-2 text-sm text-foreground">
              ...
              <input value={task.hours} onChange={(e) => updateTask(i, { hours: Number(e.target.value) })} type="number" min="0.5" max="12" step="0.5"
                className="w-16 bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-2 text-sm text-foreground text-center" />
        ...
        <div className="grid grid-cols-1 gap-4">
          <div><label className="text-xs text-zinc-500 block mb-1">成果・気づき</label>
            <textarea value={achievements} onChange={(e) => setAchievements(e.target.value)} rows={2} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-foreground resize-none focus:outline-none focus:ring-1 focus:ring-accent-500/50" /></div>
          <div><label className="text-xs text-zinc-500 block mb-1">課題・困りごと</label>
            <textarea value={issues} onChange={(e) => setIssues(e.target.value)} rows={2} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-foreground resize-none focus:outline-none focus:ring-1 focus:ring-accent-500/50" /></div>
          <div><label className="text-xs text-zinc-500 block mb-1">明日の予定</label>
            <textarea value={tomorrow} onChange={(e) => setTomorrow(e.target.value)} rows={2} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-foreground resize-none focus:outline-none focus:ring-1 focus:ring-accent-500/50" /></div>
        </div>
        ...
        <button type="submit" className="bg-accent-600 hover:bg-accent-500 text-on-accent px-6 py-2.5 rounded-lg text-sm font-medium transition-colors">日報を提出</button>
      </form>
    </PageTransition>
  );
}
