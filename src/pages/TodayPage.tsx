    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-3">
          ...
          {tasks.map((task, i) => (
            <div key={i} className="flex items-center gap-3">
              <input value={task.title} onChange={(e) => updateTask(i, { title: e.target.value })} placeholder="タスク内容"
                className="flex-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-sky-500/50" />
          ...
          <button type="button" onClick={addTask} className="flex items-center gap-1 text-sm text-sky-400 hover:text-sky-300"><Plus className="w-4 h-4" />タスクを追加</button>
        </div>
        <div className="grid grid-cols-1 gap-4">
          <div><label className="text-xs text-zinc-500 block mb-1">成果・気づき</label>
            <textarea value={achievements} onChange={(e) => setAchievements(e.target.value)} rows={2} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-white resize-none focus:outline-none focus:ring-1 focus:ring-sky-500/50" /></div>
          <div><label className="text-xs text-zinc-500 block mb-1">課題・困りごと</label>
            <textarea value={issues} onChange={(e) => setIssues(e.target.value)} rows={2} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-white resize-none focus:outline-none focus:ring-1 focus:ring-sky-500/50" /></div>
          <div><label className="text-xs text-zinc-500 block mb-1">明日の予定</label>
            <textarea value={tomorrow} onChange={(e) => setTomorrow(e.target.value)} rows={2} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-white resize-none focus:outline-none focus:ring-1 focus:ring-sky-500/50" /></div>
        </div>
        <div><label className="text-xs text-zinc-500 block mb-2">今日の調子</label>
          <div className="flex gap-3">{moods.map((m, i) => (
            <button key={i} type="button" onClick={() => setMood((i + 1) as any)}
              className={`text-2xl p-2 rounded-lg transition-all ${mood === i + 1 ? 'bg-sky-500/10 scale-110' : 'opacity-50 hover:opacity-100'}`}>{m}</button>
          ))}</div>
        </div>
        <button type="submit" className="bg-sky-600 hover:bg-sky-500 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-colors">日報を提出</button>
      </form>
    </motion.div>
  );
}
